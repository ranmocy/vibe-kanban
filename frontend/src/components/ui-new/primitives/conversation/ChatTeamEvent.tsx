...
export function ChatTeamEvent({
  ...
  const eventLabel =
    eventType === 'create'
      ? t('conversation.team.created')
      : t('conversation.team.deleted');
    ...
    >
      ...
      >
        <span className="relative shrink-0">
          <UsersThreeIcon className="size-icon-base text-low" />
        </span>
        <div className="flex-1 min-w-0">
          ...
          <span className="text-sm text-normal truncate block">{teamName}</span>
        </div>
        {onToggle && hasContent && (
          <CaretDownIcon
            className={cn(
